...
if ((int)$chk->fetchColumn()===0) {
    ...
    foreach ([['Food','#ef4444'],['Medical','#3b82f6'],['Travel','#06b6d4'],
              ['Education','#8b5cf6'],['Personal','#10b981'],['Family','#f97316'],
              ['Entertainment','#ec4899'],['Living Cost','#f59e0b']] as [$n,$c])
        $ins->execute([$uid,$n,$c]);
}
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if ($act==='add_expense'||$act==='edit_expense') {
        ...
        elseif ($eid) {
            $db->prepare("UPDATE expenses SET title=?,amount=?,category_id=?,expense_date=?
                          WHERE expense_id=? AND user_id=?")
               ->execute([$title,$amount,$cat,$date,$eid,$uid]);
            flash('Expense updated.');
        } else {
            $db->prepare("INSERT INTO expenses (user_id,category_id,title,amount,expense_date)
                          VALUES (?,?,?,?,?)")
    ...
    if ($act==='add_cat'||$act==='edit_cat') {
        ...
        $color=$_POST['cat_color']??'#f59e0b';
        $cid=(int)($_POST['cat_id']??0);
        if (!$name) { flash('Name required.','danger'); }
        elseif ($cid) {
            $db->prepare("UPDATE categories SET category_name=?,category_color=?
                          WHERE category_id=? AND user_id=?")
               ->execute([$name,$color,$cid,$uid]);
            flash('Category updated.');
        } else {
            $db->prepare("INSERT INTO categories (user_id,category_name,category_color)
                          VALUES (?,?,?)")
    ...
    // ── SAVINGS: sealed vault logic ───────────────────────────
    if ($act==='save_saving') {
        $type=$_POST['stype']??'deposit';
        $amount=(float)($_POST['amount']??0);
        $note=trim($_POST['note']??'');
        if ($amount<=0) {
            flash('Enter a valid amount.','danger');
        } elseif ($type==='withdraw') {
            // Check sealed balance before allowing withdrawal
            $st=$db->prepare("SELECT COALESCE(SUM(CASE WHEN type='deposit' THEN amount ELSE -amount END),0)
                              FROM savings WHERE user_id=?");
            $st->execute([$uid]); $bal=(float)$st->fetchColumn();
            if ($amount>$bal) {
                flash('Insufficient sealed savings. Available: '.tk($bal),'danger');
                header("Location: dashboard.php?p=savings"); exit;
            }
            $db->prepare("INSERT INTO savings (user_id,type,amount,note) VALUES (?,?,?,?)")
               ->execute([$uid,'withdraw',$amount,$note]);
            flash('৳'.number_format($amount,2).' unsealed from savings.');
        } else {
            // Deposit = seal money into vault
            $db->prepare("INSERT INTO savings (user_id,type,amount,note) VALUES (?,?,?,?)")
               ->execute([$uid,'deposit',$amount,$note]);
            flash('৳'.number_format($amount,2).' sealed into savings vault.');
        }
        header("Location: dashboard.php?p=savings"); exit;
    }
}
...
// ── SHARED DATA ───────────────────────────────────────────────
// Budget = monthly limit from budgets table
$st=$db->prepare("SELECT monthly_limit FROM budgets WHERE user_id=? AND month=? AND year=?");
$st->execute([$uid,$m,$y]);
$budget=(float)($st->fetchColumn()?:0);

// Monthly spent = ONLY expenses (savings are sealed, never counted)
$st=$db->prepare("SELECT COALESCE(SUM(amount),0) FROM expenses
                  WHERE user_id=? AND MONTH(expense_date)=? AND YEAR(expense_date)=?");
$st->execute([$uid,$m,$y]);
$monthTotal=(float)$st->fetchColumn();

// Budget remaining = limit - expenses only (savings excluded completely)
...
// Savings vault balance (sealed amount)
$st=$db->prepare("SELECT COALESCE(SUM(CASE WHEN type='deposit' THEN amount ELSE -amount END),0)
                  FROM savings WHERE user_id=?");
$st->execute([$uid]);
$savingsBalance=(float)$st->fetchColumn();
...
$inp="w-full px-3.5 py-2.5 rounded-xl text-sm text-white placeholder-white/30 outline-none transition inp-field";
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<script>tailwind.config={theme:{extend:{colors:{p:{DEFAULT:'#0f172a',2:'#1e293b'},a:'#f59e0b',g:'#10b981',b:'#06b6d4',vio:'#8b5cf6'},fontFamily:{sans:['Inter','ui-sans-serif'],mono:['JetBrains Mono','ui-monospace']}}}}</script>
...
.sidebar{background:rgba(15,23,42,.92);backdrop-filter:blur(20px);border-right:1px solid rgba(255,255,255,.07)}
.card{background:rgba(255,255,255,.05);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,.08);border-radius:1rem}
.nav-active{background:rgba(245,158,11,.15);color:#f59e0b;border:1px solid rgba(245,158,11,.25)}
.nav-item{color:rgba(255,255,255,.45);border:1px solid transparent}
...
.btn-p{background:#f59e0b;color:#0f172a;font-weight:700;border-radius:.75rem;padding:.65rem 1.25rem;font-size:.875rem;cursor:pointer;border:none;transition:.15s;display:inline-flex;align-items:center;gap:.5rem}
.btn-p:hover{background:#fbbf24}
.btn-s{background:rgba(255,255,255,.08);color:#fff;font-weight:500;border-radius:.75rem;padding:.65rem 1.25rem;font-size:.875rem;cursor:pointer;border:1px solid rgba(255,255,255,.12);transition:.15s;text-decoration:none;display:inline-flex;align-items:center}
...
<aside class="sidebar w-60 fixed top-0 left-0 bottom-0 flex flex-col z-40">
  ...
  <!-- Savings balance in sidebar -->
  <div class="px-4 py-3 mx-3 mb-2 rounded-xl" style="background:rgba(16,185,129,.12);border:1px solid rgba(16,185,129,.2)">
  ...
  <div class="px-4 py-4" style="border-top:1px solid rgba(255,255,255,.07)">
    <div class="flex items-center gap-2.5">
      <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
           style="background:rgba(245,158,11,.2);border:1px solid rgba(245,158,11,.3);color:#f59e0b">
...
// Pie: expenses only, savings excluded
$st=$db->prepare("SELECT c.category_name,c.category_color,
                  ...
                  WHERE c.user_id=?
                  GROUP BY c.category_id HAVING total>0 ORDER BY total DESC");
$st->execute([$m,$y,$uid]); $catData=$st->fetchAll();

$barL=[]; $barA=[];
for ($i=5;$i>=0;$i--) {
    $dt=new DateTime("first day of -$i month"); $barL[]=$dt->format('M');
    $s2=$db->prepare("SELECT COALESCE(SUM(amount),0) FROM expenses
                      WHERE user_id=? AND MONTH(expense_date)=? AND YEAR(expense_date)=?");
    $s2->execute([$uid,(int)$dt->format('n'),(int)$dt->format('Y')]);
    $barA[]=(float)$s2->fetchColumn();
}
...
$bColor=$budgetPct>=100?'#ef4444':($budgetPct>=90?'#f59e0b':'#10b981');
?>

<div class="flex justify-between items-center mb-6">
  <div>
    <h1 class="text-2xl font-bold">Dashboard</h1>
    <p class="text-white/35 text-sm mt-0.5"><?= date('l, F j, Y') ?></p>
  </div>
  <a href="dashboard.php?p=expenses&act=add" class="btn-p">+ Add Expense</a>
</div>

<?php if ($budget>0 && $budgetPct>=90): ?>
<div class="mb-5 px-4 py-3 rounded-xl text-sm font-semibold border
            <?= $overBudget?'bg-red-500/15 text-red-300 border-red-500/30':'bg-a/10 text-a border-a/25' ?>">
  <?= $overBudget
      ? '⚠ Budget exceeded! Spent '.tk($monthTotal).' of '.tk($budget).' limit.'
      : '⚠ '.round($budgetPct).'% of '.date('F').' budget used. '.tk($budgetLeft).' remaining.' ?>
</div>
<?php endif; ?>

<!-- 4 stat cards — savings completely separate from budget -->
<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">

  <!-- Spent this month (expenses only) -->
  <div class="stat-card" style="<?= $overBudget?'border-color:rgba(239,68,68,.3)':'' ?>">
    ...
    <div class="text-xs text-white/25 mt-1"><?= date('F Y') ?> expenses only</div>
    <?php if ($budget>0): ?>
    <div class="mt-2.5 h-1.5 rounded-full overflow-hidden" style="background:rgba(255,255,255,.08)">
      <div class="h-full rounded-full transition-all" style="width:<?= $budgetPct ?>%;background:<?= $bColor ?>"></div>
    </div>
    <div class="flex justify-between text-xs text-white/25 mt-1">
      <span><?= round($budgetPct) ?>% used</span><span>of <?= tk($budget) ?></span>
    </div>
    <?php endif; ?>
  </div>

  <!-- Budget left (expenses vs limit only) -->
  <div class="stat-card" style="<?= $overBudget?'border-color:rgba(239,68,68,.3)':($budgetPct>=90?'border-color:rgba(245,158,11,.3)':'' ) ?>">
    <div class="text-xs font-semibold text-white/35 uppercase tracking-wider mb-2">💰 Budget Left</div>
    <?php if ($budget>0): ?>
    <div class="text-2xl font-bold font-mono <?= $overBudget?'text-red-400':($budgetPct>=90?'text-a':'text-g') ?>">
      <?= $overBudget ? '-'.tk($monthTotal-$budget) : tk($budgetLeft) ?>
    </div>
    <div class="text-xs text-white/25 mt-1"><?= $overBudget?'Over limit':'Remaining budget' ?></div>
    <?php else: ?>
    <div class="text-lg font-bold text-white/30">No limit set</div>
    <a href="dashboard.php?p=profile" class="text-xs text-a hover:underline">Set budget →</a>
    <?php endif; ?>
  </div>
  ...
  <!-- Savings vault (SEALED — separate from budget) -->
  <div class="stat-card" style="background:rgba(16,185,129,.08);border-color:rgba(16,185,129,.2)">
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
  <div class="card p-5">
    ...
    <div class="text-xs text-white/30 mb-4"><?= date('F Y') ?> · Expenses: <?= tk($monthTotal) ?> · Savings not included</div>
    <?php if ($catData): ?>
  ...
  <div class="card p-5">
    ...
    <div class="text-xs text-white/30 mb-4">Monthly expense trend (savings excluded)</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
  <div class="card p-5 lg:col-span-2">
    ...
    <table>
      ...
      <tr>
        ...
        <td><?php if ($r['category_name']): ?>
          <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium"
                style="background:<?= e($r['category_color']) ?>22;color:<?= e($r['category_color']) ?>;border:1px solid <?= e($r['category_color']) ?>44">
    ...
    <?php else: ?>
    <div class="text-center py-8 text-sm text-white/25">No expenses yet. <a href="dashboard.php?p=expenses&act=add" class="text-a hover:underline">Add first →</a></div>
    <?php endif; ?>
  </div>
  ...
  <div class="card p-5" style="background:linear-gradient(135deg,rgba(16,185,129,.1),rgba(6,182,212,.06));border-color:rgba(16,185,129,.2)">
    ...
    <div class="text-xs text-white/25 mb-5">sealed balance</div>
    <a href="dashboard.php?p=savings" class="btn-p w-full justify-center" style="background:#10b981;width:100%;justify-content:center">Manage Vault</a>
    <?php
    ...
    if ($recentSav): ?>
    <div class="mt-4 pt-4 space-y-2" style="border-top:1px solid rgba(255,255,255,.08)">
      <?php foreach ($recentSav as $s): ?>
      <div class="flex justify-between items-center">
        <div>
          <span class="text-xs font-bold <?= $s['type']==='deposit'?'text-g':'text-red-400' ?>">
            <?= $s['type']==='deposit'?'🔒 Sealed':'🔓 Unsealed' ?>
          </span>
          <?php if ($s['note']): ?><div class="text-xs text-white/25 truncate w-28"><?= e($s['note']) ?></div><?php endif; ?>
        </div>
        <span class="text-xs font-mono font-bold <?= $s['type']==='deposit'?'text-g':'text-red-400' ?>">
...
new Chart(document.getElementById('barChart'),{type:'bar',
  data:{labels:<?= json_encode($barL) ?>,
    datasets:[{data:<?= json_encode($barA) ?>,
      backgroundColor:['rgba(239,68,68,.7)','rgba(59,130,246,.7)','rgba(6,182,212,.7)',
                       'rgba(139,92,246,.7)','rgba(16,185,129,.7)','rgba(245,158,11,.9)'],
      borderWidth:0,borderRadius:8}]},
  options:{...co,plugins:{legend:{display:false}},
    scales:{
      y:{beginAtZero:true,grid:{color:'rgba(255,255,255,.04)'},ticks:{color:'rgba(255,255,255,.3)',font:{family:'JetBrains Mono',size:10}}},
...
$st=$db->prepare("SELECT e.*,c.category_name,c.category_color FROM expenses e
    LEFT JOIN categories c ON c.category_id=e.category_id
    $wh ORDER BY e.expense_date DESC,e.created_at DESC LIMIT $pp OFFSET $off");
$st->execute($pr);$expenses=$st->fetchAll();
$st=$db->prepare("SELECT COALESCE(SUM(e.amount),0) FROM expenses e $wh");
$st->execute($pr);$fSum=(float)$st->fetchColumn();
?>

<div class="flex justify-between items-center mb-6">
  <div><h1 class="text-2xl font-bold">💸 Expenses</h1>
  <p class="text-white/35 text-sm mt-0.5"><?= $tot ?> records · <?= tk($fSum) ?></p></div>
  <a href="dashboard.php?p=expenses&act=add" class="btn-p">+ Add</a>
</div>
...
<div class="card p-6 max-w-lg">
  ...
  <form method="POST" action="dashboard.php">
    ...
    <input type="text" name="title" class="inp-field w-full px-3.5 py-2.5 rounded-xl text-sm mb-4"
           value="<?= e($editing['title']??'') ?>" placeholder="e.g. Lunch, Rickshaw" required autofocus>
    <div class="grid grid-cols-2 gap-3 mb-4">
      <div>
        <label class="block text-xs font-semibold text-white/35 uppercase tracking-wider mb-1.5">Amount (৳)</label>
        <input type="number" name="amount" step="0.01" min="0.01" class="inp-field w-full px-3.5 py-2.5 rounded-xl text-sm"
               value="<?= $editing?$editing['amount']:'' ?>" placeholder="0.00" required>
      </div>
      <div>
        <label class="block text-xs font-semibold text-white/35 uppercase tracking-wider mb-1.5">Date</label>
        <input type="date" name="expense_date" class="inp-field w-full px-3.5 py-2.5 rounded-xl text-sm"
               value="<?= $editing?$editing['expense_date']:date('Y-m-d') ?>" required>
      </div>
    </div>
    <label class="block text-xs font-semibold text-white/35 uppercase tracking-wider mb-1.5">
      Category <span class="normal-case font-normal text-white/20">optional</span>
    </label>
    <select name="category_id" class="inp-field w-full px-3.5 py-2.5 rounded-xl text-sm mb-5">
    ...
    <div class="flex gap-3">
      <button type="submit" class="btn-p"><?= $editing?'Update':'Save' ?></button>
      <a href="dashboard.php?p=expenses" class="btn-s">Cancel</a>
    </div>
  </form>
</div>
...
<form method="GET" action="dashboard.php" class="flex gap-2 mb-5 flex-wrap">
  ...
  <button type="submit" class="btn-s" style="padding:.5rem 1rem">Filter</button>
  <?php if ($fm||$fc): ?><a href="dashboard.php?p=expenses" class="btn-s" style="padding:.5rem 1rem">Clear</a><?php endif; ?>
</form>
...
<div class="card overflow-hidden">
  ...
  <div class="overflow-x-auto"><table>
    ...
    <tr>
      ...
      <td><?php if ($exp['category_name']): ?>
        <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium"
              style="background:<?= e($exp['category_color']) ?>22;color:<?= e($exp['category_color']) ?>;border:1px solid <?= e($exp['category_color']) ?>44">
      ...
      <td class="text-right whitespace-nowrap">
        <a href="dashboard.php?p=expenses&act=edit&id=<?= $exp['expense_id'] ?>" class="text-xs text-white/35 hover:text-a px-2 py-1 rounded transition mr-1">Edit</a>
        <form method="POST" action="dashboard.php" class="inline" onsubmit="return confirm('Delete?')">
          ...
          <button type="submit" class="text-xs text-white/35 hover:text-red-400 px-2 py-1 rounded transition">Del</button>
        </form>
      </td>
    </tr>
    <?php endforeach; ?>
  ...
  <div class="flex justify-center gap-1 py-4" style="border-top:1px solid rgba(255,255,255,.07)">
    ...
    <a href="dashboard.php?p=expenses&pg=<?= $i ?>&month=<?= e($fm) ?>&cat=<?= e($fc) ?>"
       ...
       style="<?= $i===$pg?'background:#f59e0b;color:#0f172a':'background:rgba(255,255,255,.07);color:rgba(255,255,255,.5)' ?>"><?= $i ?></a>
    <?php endfor; ?>
  </div>
  <?php endif; ?>
  <?php else: ?>
  <div class="text-center py-14 text-sm text-white/25">No expenses.<?php if (!$fm&&!$fc): ?> <a href="dashboard.php?p=expenses&act=add" class="text-a hover:underline">Add first →</a><?php endif; ?></div>
  <?php endif; ?>
</div>
<?php endif; ?>
...
<div class="flex justify-between items-center mb-6">
  ...
  <a href="dashboard.php?p=categories&act=add" class="btn-p">+ New</a>
</div>
...
<div class="card p-6 max-w-sm">
  ...
  <form method="POST" action="dashboard.php">
    ...
    <input type="text" name="cat_name" class="inp-field w-full px-3.5 py-2.5 rounded-xl text-sm mb-4"
           value="<?= e($editCat['category_name']??'') ?>" placeholder="e.g. Food" required autofocus>
    ...
    <div class="flex flex-wrap gap-2 mb-5" id="colorPicker">
      ...
      <div onclick="pickColor('<?= $c ?>')" data-c="<?= $c ?>"
           class="w-7 h-7 rounded-full cursor-pointer transition-transform hover:scale-110 <?= $sel?'ring-2 ring-white ring-offset-2':'' ?>"
           style="background:<?= $c ?>"></div>
      <?php endforeach; ?>
    </div>
    <div class="flex gap-3">
      <button type="submit" class="btn-p"><?= $editCat?'Update':'Create' ?></button>
      <a href="dashboard.php?p=categories" class="btn-s">Cancel</a>
    </div>
  </form>
</div>
<?php else: ?>
<div class="card overflow-hidden">
  ...
  <table>
    ...
    <tr>
      <td><span class="inline-flex items-center gap-2 font-semibold text-white">
        <span class="w-3 h-3 rounded-full" style="background:<?= e($c['category_color']) ?>"></span><?= e($c['category_name']) ?>
      </span></td>
  ...
  <?php else: ?>
  <div class="text-center py-14 text-sm text-white/25">No categories. <a href="dashboard.php?p=categories&act=add" class="text-a hover:underline">Create first →</a></div>
  <?php endif; ?>
</div>
<?php endif; ?>
...
// ════════════════════════════════
elseif ($page==='savings'):
$st=$db->prepare("SELECT * FROM savings WHERE user_id=? ORDER BY created_at DESC LIMIT 50");
...
<div class="mb-6">
  <h1 class="text-2xl font-bold">🏦 Savings Vault</h1>
  <p class="text-white/35 text-sm mt-0.5">Sealed from budget · Withdraw anytime</p>
</div>
...
<div class="mb-5 px-4 py-3 rounded-xl text-sm border" style="background:rgba(16,185,129,.08);border-color:rgba(16,185,129,.2);color:rgba(16,185,129,.9)">
  🔒 Savings are <strong>sealed</strong> and completely separate from your budget calculations. Depositing money seals it into the vault. Withdrawing unseals it back to you.
</div>
...
<div class="grid grid-cols-3 gap-4 mb-6">
  <div class="card p-5" style="background:linear-gradient(135deg,rgba(16,185,129,.15),rgba(6,182,212,.08));border-color:rgba(16,185,129,.3)">
    ...
    <div class="text-xs text-white/25 mt-1">available to unseal</div>
  </div>
  <div class="card p-5">
    <div class="text-xs font-semibold text-white/35 uppercase tracking-wider mb-2">↑ Total Sealed</div>
    <div class="text-2xl font-bold font-mono text-g"><?= tk($savStats['total_dep']) ?></div>
    <div class="text-xs text-white/25 mt-1">all deposits</div>
  </div>
  <div class="card p-5">
    <div class="text-xs font-semibold text-white/35 uppercase tracking-wider mb-2">↓ Total Unsealed</div>
    <div class="text-2xl font-bold font-mono text-red-400"><?= tk($savStats['total_with']) ?></div>
    <div class="text-xs text-white/25 mt-1">all withdrawals</div>
  </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-6">
  ...
  <div class="card p-6" style="border-color:rgba(16,185,129,.25)">
    ...
    <form method="POST" action="dashboard.php">
      ...
      <label class="block text-xs font-semibold text-white/35 uppercase tracking-wider mb-1.5">Amount (৳)</label>
      <input type="number" name="amount" class="inp-field w-full px-3.5 py-2.5 rounded-xl text-sm mb-3"
             step="0.01" min="0.01" placeholder="0.00" required>
      <label class="block text-xs font-semibold text-white/35 uppercase tracking-wider mb-1.5">
        Note <span class="normal-case font-normal text-white/20">optional</span>
      </label>
      <input type="text" name="note" class="inp-field w-full px-3.5 py-2.5 rounded-xl text-sm mb-4"
             placeholder="e.g. Emergency fund, Vacation">
      <button type="submit" class="btn-p w-full justify-center" style="background:#10b981;width:100%">🔒 Seal into Vault</button>
    </form>
  </div>

  <!-- Unseal money (withdraw) -->
  <div class="card p-6" style="border-color:rgba(239,68,68,.2)">
    ...
    <div class="text-xs text-white/25 mb-4">Releases sealed money back to you</div>
    <form method="POST" action="dashboard.php" onsubmit="return confirm('Unseal ৳'+document.getElementById('wamt').value+' from savings?')">
      <input type="hidden" name="act" value="save_saving">
      <input type="hidden" name="stype" value="withdraw">
      <label class="block text-xs font-semibold text-white/35 uppercase tracking-wider mb-1.5">Amount (৳)</label>
      <input type="number" id="wamt" name="amount" class="inp-field w-full px-3.5 py-2.5 rounded-xl text-sm mb-1"
             step="0.01" min="0.01" max="<?= $savingsBalance ?>" placeholder="0.00" required>
      <div class="text-xs text-white/25 mb-3">Max: <span class="font-mono font-bold text-g"><?= tk($savingsBalance) ?></span></div>
      <label class="block text-xs font-semibold text-white/35 uppercase tracking-wider mb-1.5">
        Note <span class="normal-case font-normal text-white/20">optional</span>
      </label>
      <input type="text" name="note" class="inp-field w-full px-3.5 py-2.5 rounded-xl text-sm mb-4"
             placeholder="e.g. Medical emergency, Travel">
      <button type="submit" <?= $savingsBalance<=0?'disabled':'' ?>
              class="w-full font-bold py-2.5 rounded-xl text-sm transition text-center"
              style="background:<?= $savingsBalance>0?'rgba(239,68,68,.25)':'rgba(255,255,255,.05)' ?>;color:<?= $savingsBalance>0?'#fca5a5':'rgba(255,255,255,.2)' ?>;border:1px solid <?= $savingsBalance>0?'rgba(239,68,68,.35)':'rgba(255,255,255,.08)' ?>;cursor:<?= $savingsBalance>0?'pointer':'not-allowed' ?>">
        <?= $savingsBalance>0?'🔓 Unseal from Vault':'No sealed savings' ?>
      </button>
    </form>
  </div>
</div>
...
<!-- Transaction log -->
<div class="card overflow-hidden">
  <div class="px-5 py-4 flex justify-between items-center" style="border-bottom:1px solid rgba(255,255,255,.07)">
    <div class="text-sm font-bold">Transaction History</div>
    <div class="text-xs text-white/25"><?= count($savLogs) ?> transactions</div>
  </div>
  <?php if ($savLogs): ?>
  <table>
    <thead><tr><th>Action</th><th>Note</th><th>Date & Time</th><th style="text-align:right">Amount</th></tr></thead>
    <tbody>
  ...
  <?php else: ?>
  <div class="text-center py-14 text-sm text-white/25">No transactions yet. Seal your first amount above.</div>
  <?php endif; ?>
</div>
...
$st=$db->prepare("SELECT monthly_limit FROM budgets WHERE user_id=? AND month=? AND year=?");
$st->execute([$uid,$m,$y]);$curBudget=$st->fetchColumn()?:'';
?>
...
<div class="flex flex-col gap-5 max-w-lg">
  ...
  <div class="card p-6">
    ...
    <form method="POST" action="dashboard.php">
      ...
      <input type="text" name="user_name" class="inp-field w-full px-3.5 py-2.5 rounded-xl text-sm mb-4" value="<?= e($u['user_name']) ?>" required>
      <label class="block text-xs font-semibold text-white/35 uppercase tracking-wider mb-1.5">Email</label>
      <input type="email" name="user_email" class="inp-field w-full px-3.5 py-2.5 rounded-xl text-sm mb-4" value="<?= e($u['user_email']) ?>" required>
      <label class="block text-xs font-semibold text-white/35 uppercase tracking-wider mb-1.5">Phone</label>
      <input type="text" name="user_phone" class="inp-field w-full px-3.5 py-2.5 rounded-xl text-sm mb-5" value="<?= e($u['user_phone']??'') ?>">
      <button type="submit" class="btn-p">Save Changes</button>
    </form>
  </div>

  <div class="card p-6">
    <p class="text-sm font-bold pb-4 mb-5" style="border-bottom:1px solid rgba(255,255,255,.07)">
      Monthly Budget
      <span class="text-xs font-normal text-white/25 ml-2">· expenses only, savings excluded</span>
    </p>
    <form method="POST" action="dashboard.php">
      <input type="hidden" name="act" value="save_budget">
      <div class="grid grid-cols-2 gap-3 mb-4">
        <div>
          <label class="block text-xs font-semibold text-white/35 uppercase tracking-wider mb-1.5">Month</label>
          <select name="bmonth" class="inp-field w-full px-3.5 py-2.5 rounded-xl text-sm">
            <?php for ($i=1;$i<=12;$i++): ?><option value="<?= $i ?>" <?= $i==$m?'selected':'' ?>><?= date('F',mktime(0,0,0,$i,1)) ?></option><?php endfor; ?>
          </select>
        </div>
        <div>
          <label class="block text-xs font-semibold text-white/35 uppercase tracking-wider mb-1.5">Year</label>
          <select name="byear" class="inp-field w-full px-3.5 py-2.5 rounded-xl text-sm">
      ...
      <input type="number" name="monthly_limit" class="inp-field w-full px-3.5 py-2.5 rounded-xl text-sm mb-1"
             step="0.01" min="0" value="<?= e($curBudget) ?>" placeholder="e.g. 15000">
      <p class="text-xs text-white/20 mb-5">Set 0 to remove. This limit applies to expenses only.</p>
      <button type="submit" class="btn-p">Update Budget</button>
    </form>
  </div>
  ...
  <div class="card p-6">
    ...
    <form method="POST" action="dashboard.php">
      ...
      <input type="password" name="cur_pass" class="inp-field w-full px-3.5 py-2.5 rounded-xl text-sm mb-4" required>
      <label class="block text-xs font-semibold text-white/35 uppercase tracking-wider mb-1.5">New Password</label>
      <input type="password" name="new_pass" class="inp-field w-full px-3.5 py-2.5 rounded-xl text-sm mb-4" placeholder="Min 6 characters" required>
      <label class="block text-xs font-semibold text-white/35 uppercase tracking-wider mb-1.5">Confirm New Password</label>
      <input type="password" name="new_pass2" class="inp-field w-full px-3.5 py-2.5 rounded-xl text-sm mb-5" required>
      <button type="submit" class="btn-p">Change Password</button>
    </form>
  </div>

  <div style="padding-top:.75rem;border-top:1px solid rgba(255,255,255,.07)">
    <a href="logout.php" style="background:rgba(239,68,68,.2);color:#fca5a5;font-weight:600;border-radius:.75rem;padding:.65rem 1.25rem;font-size:.875rem;border:1px solid rgba(239,68,68,.3);display:inline-flex;align-items:center;gap:.5rem">Sign Out</a>
  </div>
</div>
