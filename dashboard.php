...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'del_cat') {
        ...
        $db->prepare("UPDATE expenses SET category_id=NULL
                      WHERE category_id=? AND user_id=?")->execute([$cid,$uid]);
    ...
    if ($act === 'save_profile') {
        ...
        if (!$errs) {
            ...
            if ($chk->fetch()) {
                ...
                $db->prepare("UPDATE users SET user_name=?,user_email=?,user_phone=?
                              WHERE user_id=?")
...
// ── SHARED QUERIES ────────────────────────────────────────────
$st = $db->prepare("SELECT monthly_limit FROM budgets
...
$flash = getFlash();
$COLORS = ['#2d5a3d','#c0392b','#2980b9','#d4860a','#8e44ad',
           '#16a085','#e67e22','#7f8c8d','#e91e63','#795548'];
$inp = "w-full px-3 py-2.5 rounded-lg border border-stone-200 bg-stone-50 text-sm
        text-stone-800 placeholder-stone-300 focus:outline-none
        focus:ring-2 focus:ring-brand-mid/20 focus:border-brand-mid transition";
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
tailwind.config = {
  theme: { extend: {
    ...
    fontFamily: { sans:['DM Sans','ui-sans-serif'], mono:['DM Mono','ui-monospace'] }
  }}
}
</script>
...
<aside class="w-56 bg-white border-r border-stone-200 fixed top-0 left-0 bottom-0 flex flex-col z-40">
  ...
  <nav class="flex-1 px-3 py-3 space-y-0.5">
    ...
    <a href="dashboard.php?p=<?= $key ?>"
       class="flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-sm font-medium transition
              <?= $a ? 'bg-brand-light text-brand-mid'
  ...
  <div class="px-4 py-4 border-t border-stone-100">
    <div class="flex items-center gap-2.5">
      <div class="w-8 h-8 rounded-full bg-brand-light text-brand-mid flex items-center
...
<div class="mb-6 px-4 py-3 rounded-lg text-sm font-medium border
            <?= $flash['type']==='danger'
                ...
                : 'bg-brand-light text-brand-mid border-green-200' ?>">
  <?= e($flash['msg']) ?>
</div>
<?php endif; ?>
...
$st = $db->prepare("SELECT COUNT(*) FROM expenses
                    WHERE user_id=? AND MONTH(expense_date)=? AND YEAR(expense_date)=?");
...
<div class="flex justify-between items-center mb-7">
  ...
  <a href="dashboard.php?p=expenses&act=add"
     class="flex items-center gap-1.5 bg-brand-mid hover:bg-brand text-white
...
<div class="mb-6 px-4 py-3 rounded-lg text-sm font-medium border
  ...
  <?= $budgetPct>=100
      ? '⚠ Budget exceeded! You spent more than your monthly limit.'
      : '⚠ You\'ve used '.round($budgetPct).'% of your '.date('F').' budget.' ?>
</div>
<?php endif; ?>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-6">
  <div class="bg-white rounded-xl border border-stone-200 p-5">
    <div class="text-sm font-semibold text-stone-700 mb-4">Spending by Category</div>
    <?php if ($catData): ?>
    <div class="relative h-52"><canvas id="pieChart"></canvas></div>
    <?php else: ?>
    <div class="h-52 flex items-center justify-center text-sm text-stone-400">
      No expenses this month.
    </div>
    <?php endif; ?>
  </div>
  <div class="bg-white rounded-xl border border-stone-200 p-5">
    <div class="text-sm font-semibold text-stone-700 mb-4">Last 6 Months</div>
...
<div class="bg-white rounded-xl border border-stone-200 p-5">
      ...
      <td class="py-3">
        ...
        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full
          ...
          <span class="w-1.5 h-1.5 rounded-full"
                style="background:<?= e($r['category_color']) ?>"></span>
          <?= e($r['category_name']) ?>
        </span>
        <?php else: ?><span class="text-stone-300">—</span><?php endif; ?>
      </td>
      <td class="py-3 text-stone-400 font-mono text-xs">
        <?= date('d M', strtotime($r['expense_date'])) ?>
      </td>
      <td class="py-3 text-right font-mono font-medium text-stone-700">
        <?= tk($r['amount']) ?>
      </td>
    </tr>
    <?php endforeach; ?>
  ...
  <div class="text-center py-10 text-sm text-stone-400">
    No expenses yet.
    <a href="dashboard.php?p=expenses&act=add"
       class="text-brand-mid hover:underline ml-1">Add your first →</a>
  </div>
  <?php endif; ?>
</div>
...
const co = {
  ...
  plugins: { legend: { labels: {
    font:{family:'DM Sans',size:11}, color:'#78716c', boxWidth:9, padding:10
  }}}
};
<?php if ($catData): ?>
...
new Chart(document.getElementById('barChart'), {
  ...
  data: {
    ...
    datasets: [{ data: <?= json_encode($barA) ?>,
      backgroundColor:'rgba(45,90,61,.12)',
      borderColor:'rgba(45,90,61,.6)',
  ...
  options: { ...co,
    ...
    scales:{
      y:{ beginAtZero:true, grid:{color:'#f5f5f4'},
          ticks:{color:'#a8a29e',font:{family:'DM Mono',size:10}} },
      x:{ grid:{display:false},
          ticks:{color:'#a8a29e',font:{family:'DM Sans',size:11}} }
    }
  }
});
</script>
...
<div class="bg-white rounded-xl border border-stone-200 p-6 max-w-lg">
  <p class="text-sm font-semibold text-stone-700 mb-5">
    <?= $editing ? 'Edit Expense' : 'New Expense' ?>
  </p>
  <form method="POST" action="dashboard.php">
    <input type="hidden" name="act"
           value="<?= $editing ? 'edit_expense':'add_expense' ?>">
    <?php if ($editing): ?>
    <input type="hidden" name="expense_id" value="<?= $editing['expense_id'] ?>">
    <?php endif; ?>
    <label class="block text-xs font-semibold text-stone-400 uppercase tracking-wider mb-1.5">Title</label>
    <input type="text" name="title" value="<?= e($editing['title']??'') ?>"
           placeholder="e.g. Lunch, Rickshaw" required autofocus class="<?= $inp ?> mb-4">
    ...
    <select name="category_id" class="<?= $inp ?> mb-5">
      ...
      <option value="<?= $c['category_id'] ?>"
        <?= ($editing && $editing['category_id']==$c['category_id']) ? 'selected':'' ?>>
        <?= e($c['category_name']) ?>
      </option>
      <?php endforeach; ?>
    </select>
    <div class="flex gap-3">
      <button type="submit"
              class="px-4 py-2.5 rounded-lg text-sm font-medium
                     bg-brand-mid hover:bg-brand text-white transition">
        <?= $editing ? 'Update':'Save Expense' ?>
      </button>
      <a href="dashboard.php?p=expenses"
         class="px-4 py-2.5 rounded-lg text-sm font-medium
                bg-stone-100 hover:bg-stone-200 text-stone-700 transition">
        Cancel
      </a>
    </div>
  </form>
...
<form method="GET" action="dashboard.php" class="flex gap-2.5 mb-5 flex-wrap">
  ...
  <input type="month" name="month" value="<?= e($fm) ?>"
         class="px-3 py-2 rounded-lg border border-stone-200 bg-white text-sm
                text-stone-700 focus:outline-none focus:border-brand-mid transition">
  <select name="cat"
          class="px-3 py-2 rounded-lg border border-stone-200 bg-white text-sm
                 text-stone-700 focus:outline-none focus:border-brand-mid transition">
    ...
    <?php foreach ($allCats as $c): ?>
    <option value="<?= $c['category_id'] ?>"
      <?= $fc==$c['category_id'] ? 'selected':'' ?>>
      <?= e($c['category_name']) ?>
    </option>
    <?php endforeach; ?>
  </select>
  <button type="submit"
          class="px-4 py-2 rounded-lg bg-stone-100 hover:bg-stone-200
                 text-sm font-medium text-stone-700 transition">
    Filter
  </button>
  <?php if ($fm||$fc): ?>
  <a href="dashboard.php?p=expenses"
     class="px-4 py-2 rounded-lg bg-stone-100 hover:bg-stone-200
            text-sm font-medium text-stone-600 transition">
    Clear
  </a>
  <?php endif; ?>
</form>
...
<div class="bg-white rounded-xl border border-stone-200 overflow-hidden">
      ...
      <tr class="border-b border-stone-50 hover:bg-stone-50">
        ...
        <td class="px-5 py-3.5">
          ...
          <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full
                       bg-stone-100 text-xs font-medium text-stone-600">
            <span class="w-1.5 h-1.5 rounded-full"
                  style="background:<?= e($exp['category_color']) ?>"></span>
            <?= e($exp['category_name']) ?>
          </span>
          <?php else: ?><span class="text-stone-300">—</span><?php endif; ?>
        </td>
        <td class="px-5 py-3.5 text-stone-400 font-mono text-xs">
          <?= date('d M Y',strtotime($exp['expense_date'])) ?>
        </td>
        <td class="px-5 py-3.5 text-right font-mono font-medium text-stone-700">
          <?= tk($exp['amount']) ?>
        </td>
        <td class="px-5 py-3.5 text-right whitespace-nowrap">
          <a href="dashboard.php?p=expenses&act=edit&id=<?= $exp['expense_id'] ?>"
             class="text-xs font-medium text-stone-400 hover:text-brand-mid
                    px-2 py-1 rounded hover:bg-brand-light transition mr-1">
            Edit
          </a>
          <form method="POST" action="dashboard.php" class="inline"
            ...
            <button type="submit"
                    class="text-xs font-medium text-stone-400 hover:text-red-500
                           px-2 py-1 rounded hover:bg-red-50 transition">
              Delete
            </button>
          </form>
        </td>
  ...
  <div class="text-center py-14 text-sm text-stone-400">
    ...
    <?php if (!$fm&&!$fc): ?>
    <a href="dashboard.php?p=expenses&act=add"
       class="text-brand-mid hover:underline ml-1">Add first →</a>
    <?php endif; ?>
  </div>
  <?php endif; ?>
</div>
...
$st = $db->prepare("SELECT c.*, COUNT(e.expense_id) AS cnt,
...
<div class="bg-white rounded-xl border border-stone-200 p-6 max-w-sm">
  <p class="text-sm font-semibold text-stone-700 mb-5">
    <?= $editCat ? 'Edit':'New' ?> Category
  </p>
  <form method="POST" action="dashboard.php">
    <input type="hidden" name="act"
           value="<?= $editCat ? 'edit_cat':'add_cat' ?>">
    <?php if ($editCat): ?>
    <input type="hidden" name="cat_id" value="<?= $editCat['category_id'] ?>">
    <?php endif; ?>
    <label class="block text-xs font-semibold text-stone-400 uppercase tracking-wider mb-1.5">Name</label>
    <input type="text" name="cat_name"
           value="<?= e($editCat['category_name']??'') ?>"
           placeholder="e.g. Food, Transport" required autofocus
           class="<?= $inp ?> mb-4">
    <label class="block text-xs font-semibold text-stone-400 uppercase tracking-wider mb-2">Colour</label>
    <input type="hidden" name="cat_color" id="catColor"
           value="<?= e($editCat['category_color']??'#2d5a3d') ?>">
    <div class="flex flex-wrap gap-2 mb-5" id="colorPicker">
      <?php foreach ($COLORS as $c):
        $sel = ($editCat && $editCat['category_color']===$c)
              || (!$editCat && $c==='#2d5a3d');
      ?>
      <div onclick="pickColor('<?= $c ?>')" data-c="<?= $c ?>"
           class="w-6 h-6 rounded-full cursor-pointer transition-transform
                  hover:scale-110 <?= $sel?'ring-2 ring-offset-1 ring-stone-700':'' ?>"
    ...
    <div class="flex gap-3">
      ...
      <a href="dashboard.php?p=categories"
         class="px-4 py-2.5 rounded-lg text-sm font-medium
                bg-stone-100 hover:bg-stone-200 text-stone-700 transition">
        Cancel
      </a>
...
<div class="bg-white rounded-xl border border-stone-200 overflow-hidden">
  ...
  <table class="w-full text-sm">
    ...
    <tr class="border-b border-stone-50 hover:bg-stone-50">
      <td class="px-5 py-3.5">
        <span class="inline-flex items-center gap-2 font-medium text-stone-700">
          <span class="w-2.5 h-2.5 rounded-full flex-shrink-0"
                style="background:<?= e($c['category_color']) ?>"></span>
          <?= e($c['category_name']) ?>
        </span>
      </td>
      ...
      <td class="px-5 py-3.5 text-right whitespace-nowrap">
        <a href="dashboard.php?p=categories&act=edit&id=<?= $c['category_id'] ?>"
           class="text-xs font-medium text-stone-400 hover:text-brand-mid
                  px-2 py-1 rounded hover:bg-brand-light transition mr-1">
          Edit
        </a>
        <form method="POST" action="dashboard.php" class="inline"
              onsubmit="return confirm('Delete? Linked expenses will be uncategorized.')">
  ...
  <div class="text-center py-14 text-sm text-stone-400">
    No categories yet.
    <a href="dashboard.php?p=categories&act=add"
       class="text-brand-mid hover:underline ml-1">Create first →</a>
  </div>
  <?php endif; ?>
</div>
<?php endif; ?>
...
<div class="flex flex-col gap-5 max-w-lg">
  ...
  <div class="bg-white rounded-xl border border-stone-200 p-6">
    <p class="text-sm font-semibold text-stone-700 pb-4 mb-5 border-b border-stone-100">
      Account Info
    </p>
    <form method="POST" action="dashboard.php">
  ...
  <div class="bg-white rounded-xl border border-stone-200 p-6">
    ...
    <form method="POST" action="dashboard.php">
      ...
      <input type="number" name="monthly_limit" step="0.01" min="0"
             value="<?= e($curBudget) ?>" placeholder="e.g. 15000"
             class="<?= $inp ?> mb-1">
  ...
  <div class="bg-white rounded-xl border border-stone-200 p-6">
    ...
    <form method="POST" action="dashboard.php">
      ...
      <input type="password" name="new_pass" placeholder="Min 6 characters"
             required class="<?= $inp ?> mb-4">
  ...
  <div class="pt-2 border-t border-stone-100">
    <a href="logout.php"
       class="inline-flex px-4 py-2.5 rounded-lg text-sm font-medium
